...
- `briefing/minis/NN.html` — the pre-built, verified daily mini crossword bank (**The Mini**, a 4×4). The routine inlines one per day, chosen by date. Built offline by `tools/mini-generator/generate.py`; to refill, add words there, rerun it, and bump `BANK_SIZE` (currently 19) in the routine's selector.
